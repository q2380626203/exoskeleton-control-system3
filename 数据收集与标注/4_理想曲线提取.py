    ...
    def 可视化理想曲线(self, 理想曲线, 输出文件='理想曲线.png'):
        ...
        for idx, 阶段类型 in enumerate(阶段顺序):
            ...
            if 阶段类型 in 理想曲线:
                ...
                # 绘制平均曲线
                ax.plot(x, 平均曲线, color=颜色[idx], linewidth=2, label='平均曲线')
                ...
                ax.set_title(f'{self.阶段名称[阶段类型]}阶段理想曲线 (n={数据["count"]})',
        ...
        with open(输出文件, 'w', encoding='utf-8') as f:
            ...
            for 阶段类型, 阶段英文 in 阶段映射.items():
                if 阶段类型 in 理想曲线:
                    ...
                    # 写注释
                    f.write(f"// {self.阶段名称[阶段类型]}阶段理想速度曲线\n")
