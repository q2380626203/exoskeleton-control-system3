...
// static const char *TAG = "BUTTON_DETECTOR";  // 运行时日志已禁用，TAG未使用
 ...
 * @note 配置GPIO引脚为输入模式，初始化两个检测器（助力增加、助力减少）
 */
void button_detector_init(gpio_num_t assist_up_pin, gpio_num_t assist_down_pin) {
    ...
    // ESP_LOGI(TAG, "GPIO%d: 助力增加, GPIO%d: 助力减少", assist_up_pin, assist_down_pin);  // 运行时日志已禁用
}
